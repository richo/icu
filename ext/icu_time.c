		...
		while (*isoCountries != NULL) {
			if (TYPE(countries) == T_SYMBOL || (TYPE(countries) == T_ARRAY && rb_ary_includes(countries, rb_str_new2(*isoCountries)))) {
				if (group) {
					...
					if (RARRAY_LEN(RARRAY_PTR(optgroup)[1])) {
						ruby_qsort(RARRAY_PTR(RARRAY_PTR(optgroup)[1]), RARRAY_LEN(RARRAY_PTR(optgroup)[1]), sizeof(VALUE), collateByDisplayName, collator);
